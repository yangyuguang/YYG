...
public class SelectImageCanPhotographAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        try {
            ...
            if (convertView == null) {
                vh = new ViewHolder();
                convertView = LayoutInflater.from(mContext).inflate(mContext.getResources().getIdentifier("pami_select_img_gridview_item", "layout", mContext.getPackageName()), parent, false);
                vh.itemImage = (ImageView) convertView.findViewById(mContext.getResources().getIdentifier("itemImage", "id", mContext.getPackageName()));
                vh.itemSelect = (ImageButton) convertView.findViewById(mContext.getResources().getIdentifier("itemSelect", "id", mContext.getPackageName()));
            ...
            if(position == 0){
                ...
                if (selects.contains(filePath)) {
                    vh.itemSelect.setImageResource(mContext.getResources().getIdentifier("pami_checkbox_is_select", "drawable", mContext.getPackageName()));
